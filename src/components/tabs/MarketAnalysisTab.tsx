...
export const MarketAnalysisTab: React.FC<MarketAnalysisTabProps> = ({
  ...
  const suspiciousItems = materialPrices.filter(
    (p) => p.status === "suspicious"
  ).length;
  const overpricedItems = materialPrices.filter(
      ...
      case "suspicious":
        return (
          <Badge variant="destructive" className="text-sm">
            ⚠️ {t("suspicious").toUpperCase()}
          </Badge>
        );
      case "overpriced":
        return (
          <Badge variant="destructive" className="text-sm bg-orange-600">
            🔴 {t("overpriced").toUpperCase()}
          </Badge>
        );
      case "underpriced":
        return (
          <Badge variant="secondary" className="text-sm">
            🟡 {t("underpriced").toUpperCase()}
          </Badge>
        );
      case "fair":
        return (
          ...
          {suspiciousItems > 0 && (
            <Alert variant="destructive" className="border-2">
              ...
              <AlertDescription className="text-base font-semibold">
                ⚠️ <strong>{suspiciousItems}</strong> material(s) have
                SUSPICIOUS pricing (30%+ above market rate)
              </AlertDescription>
            </Alert>
          )}
          {overpricedItems > 0 && (
            <Alert className="border-2 border-orange-500 bg-orange-50">
              ...
              <AlertDescription className="text-base font-semibold text-orange-800">
                🔴 <strong>{overpricedItems}</strong> material(s) are OVERPRICED
                (15-30% above market rate)
              </AlertDescription>
            </Alert>
          )}
          {fairItems > 0 && (
            <Alert className="border-2 border-green-500 bg-green-50">
              ...
              <AlertDescription className="text-base font-semibold text-green-800">
                ✅ <strong>{fairItems}</strong> material(s) have FAIR pricing
          ...
          >
            ...
            <CardContent className="space-y-4">
                ...
                <div
                  className={`p-3 rounded-lg border ${
                    price.status === "suspicious"
                      ? "bg-red-100 dark:bg-red-950/50 border-red-300"
                      : price.status === "overpriced"
                      ? "bg-orange-100 dark:bg-orange-950/50 border-orange-300"
              ...
              <div className="text-xs text-gray-500 space-y-1">
                <p>
                  <strong>Source:</strong> {price.source}
                </p>
                <p>
                  <strong>Updated:</strong> {price.lastUpdated}
                </p>
              </div>
            </CardContent>
          </Card>
